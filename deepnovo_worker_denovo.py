...
class WorkerDenovo(object):
  ...
  def __init__(self):
    ...
    self.neighbor_center = deepnovo_config.neighbor_size//2
  ...
  def _extend_peak(self,
    ...
    ms1_profile_list = [x["ms1_profile"] for x in spectrum_batch]
    input_feed = {}
    input_feed[model.input_dict["spectrum_holder"].name] = spectrum_holder_array
    output_feed = model_lstm_state0
    # lstm_state0_batch format is ((c_1, h_1), ..., (c_n, h_n)) for n-layer LSTM,
    #   where each c_i or h_i is numpy array [batch_size, num_units]
    lstm_state0_batch = session.run(fetches=output_feed, feed_dict=input_feed)
    ...
    for spectrum_id in xrange(spectrum_batch_size):
      ...
      path["score_list"] = [0.0]
      path["score_sum"] = 0.0
      path["lstm_state"] = [(c[spectrum_id], h[spectrum_id]) for (c, h) in lstm_state0_batch]
    ...
    while True:
      ...
      block_AAid_1 = [] # nobi
      block_AAid_2 = [] # nobi
      block_state0 = [([],[]) for x in range(deepnovo_config.num_layers)] # nobi
      block_state = [([],[]) for x in range(deepnovo_config.num_layers)] # nobi
      ...
      for entry_index, search_entry in enumerate(active_search_list):
        ...
        ms1_profile = ms1_profile_list[spectrum_id]
        lstm_state0 = [(c[spectrum_id], h[spectrum_id]) for (c, h) in lstm_state0_batch]
        ...
        for path in current_path_list:
          ...
          for layer in range(deepnovo_config.num_layers):
            block_state0[layer][0].append(lstm_state0[layer][0]) # nobi
            block_state0[layer][1].append(lstm_state0[layer][1]) # nobi
            block_state[layer][0].append(lstm_state[layer][0])
            block_state[layer][1].append(lstm_state[layer][1])
          block_candidate_intensity.append(candidate_intensity)
          block_ms1_profile.append(ms1_profile)
      ...
      if block_AAid_1:
        ...
        block_state0 = [(np.array(c), np.array(h)) for (c, h) in block_state0] # nobi
        block_state = [(np.array(c), np.array(h)) for (c, h) in block_state]
        block_candidate_intensity = np.array(block_candidate_intensity)
        block_ms1_profile = np.array(block_ms1_profile)

        input_feed = {}
        input_feed[model.input_dict["AAid"][0].name] = block_AAid_1 # nobi
        input_feed[model.input_dict["AAid"][1].name] = block_AAid_2 # nobi
        for layer in range(deepnovo_config.num_layers):
          input_feed[model.input_dict["lstm_state0"][layer][0].name] = block_state0[layer][0] # nobi
          input_feed[model.input_dict["lstm_state0"][layer][1].name] = block_state0[layer][1] # nobi
          input_feed[model.input_dict["lstm_state"][layer][0].name] = block_state[layer][0]
          input_feed[model.input_dict["lstm_state"][layer][1].name] = block_state[layer][1]
        input_feed[model.input_dict["intensity"].name] = block_candidate_intensity
        input_feed[model.input_dict["ms1_profile"].name] = block_ms1_profile

        output_feed = [model_output_log_prob, model_lstm_state]

        current_log_prob, current_state = session.run(
            output_feed,
            input_feed)
      ...
      for entry_index, search_entry in enumerate(active_search_list):
        ...
        for index in xrange(block_index, block_index + search_entry_size[entry_index]):
          for AAid in block_knapsack_candidate[index]:
            ...
            new_path["lstm_state"] = [(c[index], h[index]) for (c, h) in current_state]
  ...
  def _select_peak(self, spectrum_batch):
    ...
    for spectrum in spectrum_batch:
      ...
      argmax_location = np.argpartition(-spectrum_forward[self.neighbor_center, :precursor_mass_C_location], self.num_position)[:self.num_position] # pylint: disable=line-too-long
  ...
  def _select_sequence(self, spectrum_batch, top_candidate_batch):
    ...
    for spectrum_id in xrange(spectrum_batch_size):
      ...
      if not candidate_list: # cannot find any peptide
        ...
        score_array = np.array([x["score"]/len(x["sequence"]) for x in candidate_list])
        if (len(candidate_list) > self.topk_output):
          topk_index = np.argpartition(-score_array, self.topk_output)[:self.topk_output] # pylint: disable=line-too-long
          predicted_list = [candidate_list[index] for index in topk_index]
        else:
          predicted_list = candidate_list
        predicted_batch[spectrum_id]["score"] = [predicted["score"]/len(predicted["sequence"])
